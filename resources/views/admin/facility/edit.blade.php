            ...
            <form action="{{ route('admin.facility.update', $facility->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-6 mt-4">
                    <div class="row">
                        ...
                        <div class="col-sm-5">
                            @if (file_exists(public_path('storage/facilities/'.$facility->image)))
                                <img src="{{asset('storage/facilities/'.$facility->image)}}" class="admin-edit-image">
                            @else
                    ...
                    <div>
                        <label class="text-gray-700" for="name">NAMA FASILITAS</label>
                        <input class="form-input w-full mt-2 rounded-md bg-gray-200 focus:bg-white" type="text" name="title" value="{{ old('title', $facility->title) }}" placeholder="Jabatan">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="15">{{ old('description', $facility->description) }}</textarea>
                        @error('description')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <p class="text-gray-600 text-sm">{{ $message }}</p>
